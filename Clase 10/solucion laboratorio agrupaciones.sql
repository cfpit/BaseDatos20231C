...
select 		ciudad,
			count(nombre) as 'cantidad de clientes' 
from 		clientes
group by	ciudad;
...
select 		ciudad,
			count(nombre) as 'cantidad de clientes' 
from 		clientes
where		ciudad not like '%Dolores%'
group by	ciudad;

-- 3- Cual es la cantidad de clientes con teléfono no nulo, 
-- de cada ciudad, ordenados por ciudad en forma alfabética:
select 		ciudad,
			count(nombre) as 'cantidad de clientes' 
from 		clientes
...
-- 6- Informar el promedio del valor de compra (redondeado a 2 decimales) 
-- agrupados por ciudad (No incluir a la ciudad de La Falda) 
-- cuyo promedio sea superior a 20 $. Ordenar por promedio en forma 
-- descendente:
select		ciudad,
			round(avg(montocompra),2) as promedio
from		clientes
where		ciudad not like '%falda%'
group by	ciudad
having		avg(montocompra) > 20
order by	2 desc;


-- 7- Informar la cantidad de clientes que no residan en la ciudad de 
-- Córdoba, ordenados por cantidad de clientes, de mayor a menor:
